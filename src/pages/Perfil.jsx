...
import { Settings, User, Heart, MessageCircle, Repeat2, ArrowLeft, GraduationCap, BookOpen, Layers } from 'lucide-react';
...
export default function Perfil() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      async function carregarPerfilEMétricas() {
        if (!perfilId) return;
        setCarregando(true);
        ...
        try {
          ...
          // Filtrar comentários e avisos do mural
          ...
          // 5. Buscar curtidas, reposts e comentários do banco para mapeamento detalhado
          ...
          const mappedUserPosts = postsReais.map(post => {
            ...
            const postCommentsCount = commentsData ? commentsData.filter(c => c.title === `COMMENT:${post.id}`).length : 0;
          ...
          if (userReposts && userReposts.length > 0) {
            ...
            // Filtrar comentários e avisos do mural de reposts
            const repostsReais = (repostedPosts || []).filter(post => {
              if (!post.title) return true;
              return !post.title.startsWith('COMMENT:') && !post.title.startsWith('AVISO:');
            });

            // Obter detalhes de perfil dos autores originais para reposts
  ...
  const handleLike = async (postId) => {
    ...
    // Toggle no array de posts do usuário
    ...
    // Toggle no array de reposts do usuário
  ...
  return (
    <div className="w-full max-w-[620px] mx-auto md:pt-2">
      ...
      {/* 1. TOP BAR DO PERFIL */}
      <div className="flex items-center justify-between px-5 py-3.5 sticky top-0 bg-white/80 backdrop-blur-md z-10 border-b border-gray-50/50">
        ...
        <div className="flex items-center gap-4 text-gray-800">
          {isDono && (
            <button onClick={() => navigate('/configuracoes')} className="p-1.5 text-gray-400 hover:text-black hover:bg-gray-50 rounded-lg transition-all cursor-pointer">
              <Settings size={20} />
            </button>
          )}
        </div>
      ...
      {/* 2. GRADIENT HEADER BANNER (ESTILO PREMIUM) */}
      <div className="relative h-36 w-full bg-gradient-to-r from-[#e6f4ff] via-[#f0f3ff] to-[#f6f0ff] rounded-3xl overflow-hidden border border-gray-100 flex items-end">
        {/* Círculos abstratos decorativos */}
      ...
      <div className="px-5 pb-5 space-y-4 relative">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 sm:gap-6 -mt-10 md:-mt-12 relative z-10">
          ...
          <div className="flex-1 flex justify-around text-center pb-2 pl-0 sm:pl-4">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[11px] text-gray-400 font-light lowercase font-sans">seguidores</span>
            </div>
            <div className="flex flex-col">
        ...
        {/* 4. DADOS DE IDENTIFICAÇÃO E BADGES ACADÊMICOS */}
        <div className="space-y-2">
          <div className="flex items-center gap-2 flex-wrap">
            <h2 className="text-[16px] font-bold text-gray-950">{perfil?.nome}</h2>
            
            {/* Badges estilizados de papel acadêmico */}
      ...
      {/* 6. SELETOR DE ABAS (POSTS / REPOSTS) */}

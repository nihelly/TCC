...
// Posts de exemplo caso o feed do banco esteja vazio
const postsExemplo = [
  {
    id: 'ex1',
    user_id: '75a387bd-ec8a-4937-950b-4b437e0f1be5', // ID de Ana Silva no banco
    author_handle: '@aluno.exemplo',
    title: 'DICA DE TCC',
    content: 'Lembrete: A primeira entrega do projeto de TCC com a introdução e fundamentação teórica deve ser enviada até o final do mês!',
    image_url: null,
    likesCount: 12,
    commentsCount: 3,
    repostsCount: 2,
  ...
  {
    id: 'ex2',
    user_id: '30c2ad6f-ceaf-4f4f-90d4-17c2a6116bfd', // ID do Prof. Dr. Roberto no banco
    author_handle: '@prof.roberto',
    title: 'SEMINÁRIO IHC',
    content: 'Os slides da apresentação sobre design centrado no utilizador e heurísticas de Jakob Nielsen já estão disponíveis no material da disciplina.',
    image_url: null,
    likesCount: 45,
    commentsCount: 8,
    repostsCount: 14,
...
export default function Feed() {
  ...
  useEffect(() => {
    async function inicializarFeed() {
      try {
        ...
        // Filtra posts de comentários e de avisos (com prefixo COMMENT: ou AVISO:)
        finalPosts = finalPosts.filter(post => {
          if (!post.title) return true;
          return !post.title.startsWith('COMMENT:') && !post.title.startsWith('AVISO:');
        });

        // Se o banco estiver sem posts reais, carrega os posts de exemplo
        const isEmpty = finalPosts.length === 0;
        const postsToProcess = isEmpty ? postsExemplo : finalPosts;
        ...
        // Obter comentários do banco para contagem compartilhada
        ...
        const userIds = [...new Set(postsToProcess.map(p => p.user_id).filter(Boolean))];
        let profilesMap = {};
        if (userIds.length > 0) {
        ...
        const mappedPosts = postsToProcess.map(post => {
          ...
          let liked = user ? postLikes.some(l => l.user_id === user.id) : false;
          let reposted = user ? postReposts.some(r => r.user_id === user.id) : false;
          let likesCount = postLikes.length;
          let repostsCount = postReposts.length;

          // Se for post de exemplo, carrega persistência local do localStorage
          if (post.id.startsWith('ex')) {
            const localLiked = localStorage.getItem(`educonnect_liked_${post.id}_${user?.id || 'anon'}`) === 'true';
            const localReposted = localStorage.getItem(`educonnect_reposted_${post.id}_${user?.id || 'anon'}`) === 'true';
            
            liked = localLiked;
            reposted = localReposted;

            const savedLikesCount = localStorage.getItem(`educonnect_likes_count_${post.id}`);
            likesCount = savedLikesCount ? parseInt(savedLikesCount, 10) : post.likesCount;

            const savedRepostsCount = localStorage.getItem(`educonnect_reposts_count_${post.id}`);
            repostsCount = savedRepostsCount ? parseInt(savedRepostsCount, 10) : post.repostsCount;
          }

          const postCommentsCount = post.id.startsWith('ex')
            ? (localStorage.getItem(`educonnect_comments_${post.id}`)
                ? JSON.parse(localStorage.getItem(`educonnect_comments_${post.id}`)).length
                : (post.commentsCount || 0))
            : (commentsData ? commentsData.filter(c => c.title === `COMMENT:${post.id}`).length : 0);
  ...
  const handleLike = async (postId) => {
    ...
    // Se for post de exemplo, salva no localStorage e retorna
    if (postId.startsWith('ex')) {
      const finalCount = jaCurtido ? Math.max(0, post.likesCount - 1) : post.likesCount + 1;
      localStorage.setItem(`educonnect_liked_${postId}_${currentUser.id}`, String(!jaCurtido));
      localStorage.setItem(`educonnect_likes_count_${postId}`, String(finalCount));
      return;
    }
  ...
  const handleRepost = async (postId) => {
    ...
    // Se for post de exemplo, salva no localStorage e retorna
    if (postId.startsWith('ex')) {
      const finalCount = jaRepostado ? Math.max(0, post.repostsCount - 1) : post.repostsCount + 1;
      localStorage.setItem(`educonnect_reposted_${postId}_${currentUser.id}`, String(!jaRepostado));
      localStorage.setItem(`educonnect_reposts_count_${postId}`, String(finalCount));
      return;
    }
  ...
  // 4. Carrega comentários do post selecionado do Supabase
  useEffect(() => {
    const timer = setTimeout(() => {
      ...
      // Se for post de exemplo, carrega de localStorage
      if (comentariosPostId.startsWith('ex')) {
        const salvos = localStorage.getItem(`educonnect_comments_${comentariosPostId}`);
        const parsed = salvos ? JSON.parse(salvos) : [];
        const mapped = parsed.map(c => ({
          ...c,
          user_id: c.user_id || (comentariosPostId === 'ex1' ? '75a387bd-ec8a-4937-950b-4b437e0f1be5' : '30c2ad6f-ceaf-4f4f-90d4-17c2a6116bfd')
        }));
      ...
      async function carregarComentarios() {
        try {
          const { data, error } = await supabase
            .from('posts')
            .select('*')
            .eq('title', `COMMENT:${comentariosPostId}`)
          ...
          const comentariosFormatados = (data || []).map(c => {
            ...
            return {
              id: c.id,
              user_id: c.user_id,
              autor: p?.nome ? `@${p.nome.toLowerCase().replace(/\s+/g, '')}` : (c.author_handle || '@usuario'),
          ...
          console.error('Erro ao carregar comentários do banco:', err);
          // Fallback local se der erro de banco
          const salvos = localStorage.getItem(`educonnect_comments_${comentariosPostId}`);
          setComentarios(salvos ? JSON.parse(salvos) : []);
        }
      }
  ...
  const handleAdicionarComentario = async (e) => {
    ...
    // Se for post de exemplo, salva apenas em localStorage local
    if (comentariosPostId.startsWith('ex')) {
      const salvos = localStorage.getItem(`educonnect_comments_${comentariosPostId}`);
      const listaSalva = salvos ? JSON.parse(salvos) : [];
      localStorage.setItem(`educonnect_comments_${comentariosPostId}`, JSON.stringify([...listaSalva, comentarioOtimista]));
      return;
    }

    try {
      // Inserir comentário no Supabase (como post formatado)
      const { data: insertData, error } = await supabase
        .from('posts')
        .insert({
          title: `COMMENT:${comentariosPostId}`,
      ...
      // Salva em localStorage como fallback de cache rápido
      const salvos = localStorage.getItem(`educonnect_comments_${comentariosPostId}`);
      const listaSalva = salvos ? JSON.parse(salvos) : [];
      localStorage.setItem(`educonnect_comments_${comentariosPostId}`, JSON.stringify([...listaSalva, comentarioOtimista]));
      ...
      console.error('Erro ao adicionar comentário no banco:', err);
              ...
              <div className="flex items-center justify-between px-5 pt-4 pb-2">
                ...
                >
                  ...
                  <span className="text-[13px] text-gray-500 font-medium group-hover:text-black group-hover:underline transition-all">
                    {post.authorHandle || post.author_handle || '@aluno.exemplo'}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-end animate-in fade-in duration-200">
          ...
          <div className="relative w-full max-w-[420px] bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300 z-50">
            ...
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-[14px] font-bold text-gray-950 tracking-wider uppercase">Comentários</h3>
              <button 
